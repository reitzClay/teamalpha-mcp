...
Stale Branches (>30 days): 10
  ...
  Uncommitted Changes (⚠️ CRITICAL in production):
    M  onfig/nginx/conf.d/app.conf
     M data/mosquitto_prod/data/mosquitto.db
...
Prod Build: 22 branches total
  ...
  - Stale branches: 10
    Stale Prod Branches:
      • prod                           (83 days old)
      • prodReady                      (90 days old)
      • ftanon                         (90 days old)
      • devProd                        (108 days old)
      • main                           (109 days old)

🚨 CRITICAL ISSUES DETECTED
--------------------------------------------------------------------------------
1. ⚠️  PRODUCTION UNCOMMITTED CHANGES: 6 files modified
    This breaks reproducibility and makes rollbacks difficult.
    Files: onfig/nginx/conf.d/app.conf, data/mosquitto_prod/data/mosquitto.db, docker-compose.prod.yml

2. 🌿 BRANCH SPRAWL: 30 total branches across both builds
    Dead branches create confusion and testing overhead.

3. ⏳ STALE BRANCHES: 13 branches older than 30 days
    These should be archived or deleted.

4. 🐳 DOCKER ISOLATION PROBLEM: Running prod containers blocks dev testing
